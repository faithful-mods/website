...
export async function upload(file: File, path: `${string}/` = '/'): Promise<string> {
	...
	console.log('Uploaded here : ', filePath);
	console.log('Stored as     : ', join(fileDirPub, `${uuid}_${file.name}`));
	return encodeURI(join(fileDirPub, `${uuid}_${file.name}`));
}
...
export async function remove(publicPath: `${typeof FILE_DIR}/${string}`): Promise<void> {
	...
	console.log('Deleting as   : ', publicPath);
