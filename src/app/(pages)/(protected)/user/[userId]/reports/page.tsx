...
import { getReportsOfUser, getReportsReasons, reportSomeone, updateReportStatus } from '~/server/data/reports';
import { getPublicUsers, getUserById, updateUserRole } from '~/server/data/user';
...
export default function UserReportsPage() {
	...
	const [localReports, setReports] = useState<Report[]>([]);
	...
	useEffectOnce(() => {
		getUserById(userId)
			.then((user) => {
				...
				getPublicUsers()
					.then((u) => {
						setUsers(u.filter((uu) => uu.id !== user.id));
					});
			});

		getReportsOfUser(userId).then(setReports);
		getReportsReasons().then(setReasons);
	});
		...
		<Stack gap="sm">
			{self && (
				<Stack gap="md">
					...
					<Group gap="md" align="start">
						<Select
							style={{ width: width <= BREAKPOINT_MOBILE_LARGE ? '100%' : 'calc((100% - var(--mantine-spacing-md)) * .5)' }}
							limit={10}
					...
					<Group>
						...
						{form.isValid() && (
							<Text size="sm" ta={width <= BREAKPOINT_MOBILE_LARGE ? 'center' : 'left'}>
			...
			<Stack gap="md">
				<Stack gap={0}>
					...
					{!self && (
						<Group justify="end">
							{localUser && localUser.role !== UserRole.BANNED && (
								<Button
									mt="sm"
									variant="gradient"
									gradient={GRADIENT_DANGER}
